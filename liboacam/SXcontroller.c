...
static int	_latchFrame ( SX_STATE*, unsigned int, unsigned int,
			unsigned int );
static int	_readFrame ( SX_STATE*, unsigned char*, unsigned int );
...
_doReadExposure ( SX_STATE* cameraInfo )
{
  unsigned char*	evenFrame;
  unsigned char*	oddFrame;
  unsigned int		halfFrameSize, rowLength, numRows;
...
static int
_readFrame ( SX_STATE* cameraInfo, unsigned char* buffer, unsigned int length )
{
