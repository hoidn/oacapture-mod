...
static QString	fileFormats[MAX_FILE_FORMATS] = {
    "", "AVI", "SER", "TIFF", "FITS", "MOV"
};
#else
#define	MAX_FILE_FORMATS	5
static QString	fileFormats[MAX_FILE_FORMATS] = {
    "", "AVI", "SER", "TIFF", "MOV"
};
#endif
...
CaptureWidget::CaptureWidget ( QWidget* parent ) : QGroupBox ( parent )
{
  ...
  haveFITS = haveTIFF = haveSER = haveMOV = 1;
      ...
      if (( type = state.camera->hasControl ( c ))) {
        ...
        switch ( c ) {
          case OA_CAM_CTRL_EXPOSURE_ABSOLUTE:
          {
            ...
            const char *expText = expTextStr.toStdString().c_str();
            settings << tr ( expText ).toStdString().c_str();
            break;
          }
