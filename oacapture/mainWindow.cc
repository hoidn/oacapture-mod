...
MainWindow::readConfig ( void )
{
  ...
  if ( !config.saveSettings ) {
    ...
    config.controlsOnRight = settings.value ( "controlsOnRight", 0 ).toInt();
...
MainWindow::createMenus ( void )
{
  ...
  hideControls = new QAction ( QIcon ( ":/icons/mask.png" ),
      tr ( "Hide controls" ), this );
  hideControls->setCheckable ( true );
  hideControls->setChecked ( 0 );
  connect ( hideControls, SIGNAL ( changed()), this,
      SLOT ( doHideControls()));
  ...
  optionsMenu->addAction ( hideControls );
...
void
MainWindow::doHideControls ( void )
{
  int hideControlsState = hideControls->isChecked() ? 1 : 0;

  if ( hideControlsState ) {
    cameraWidget->hide();
    imageWidget->hide();
    zoomWidget->hide();
    controlWidget->hide();
...
MainWindow::createPreviewWindow()
{
  ...
  // previewScroller->setMinimumSize ( 640, 480 );
