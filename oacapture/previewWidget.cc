...
PreviewWidget::updatePreview ( void* args, void* imageData, int length )
{
  ...
  if (! zmq_push(imageData)) {
    qWarning() << "ZMQ operation failed.";
  }
...
int
zmq_push(void *imageData) 
{
    // send frame to ZMQ socket
    printf("pushing a frame\n");
    asi_info info;
    info.height = config.imageSizeY;
    info.width = config.imageSizeX;
    // = asi_info{config.imageSizeX, config.imageSizeY};
    return zmq_pub_frame(imageData, &info);
}
