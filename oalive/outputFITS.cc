...
int
OutputFITS::addFrame ( void* frame, const char* timestampStr )
{
  unsigned char* s;
  unsigned char* t;
  int i, status = 0;
  ...
  if ( timestampStr ) {
    ...
    fits_write_key_str ( fptr, "DATE-OBS", dateStr.toStdString().c_str(), "",
        &status );
  }
  ...
  if ( config.fitsObserver != "" ) {
    fits_write_key_str ( fptr, "OBSERVER",
        config.fitsObserver.toStdString().c_str(), "", &status );
  }
  if ( config.fitsObject != "" ) {
    fits_write_key_str ( fptr, "OBJECT",
        config.fitsObject.toStdString().c_str(), "", &status );
  }
  if ( config.fitsTelescope != "" ) {
    fits_write_key_str ( fptr, "TELESCOP",
        config.fitsTelescope.toStdString().c_str(), "", &status );
  }
  if ( config.fitsInstrument != "" ) {
    fits_write_key_str ( fptr, "INSTRUME",
        config.fitsInstrument.toStdString().c_str(), "", &status );
  }
  if ( config.fitsComment != "" ) {
    fits_write_comment ( fptr, config.fitsComment.toStdString().c_str(),
        &status );
  }
